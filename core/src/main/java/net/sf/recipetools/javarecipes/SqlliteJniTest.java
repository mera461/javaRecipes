package net.sf.recipetools.javarecipes;

/*
import SQLite.*;

public class SqlliteJniTest implements SQLite.Callback, SQLite.Function,
	...
	try {
	    db.open("db", 0666);
	    System.out.println("DB version: " + db.version());
	    db.interrupt();
	    db.busy_timeout(1000);
	    db.busy_handler(null);
	    db.create_function("myregfunc", -1, new SqlliteJniTest());
	    db.function_type("myregfunc", Constants.SQLITE_TEXT);
	    db.create_aggregate("myaggfunc", 1, new SqlliteJniTest());
	    db.function_type("myaggfunc", Constants.SQLITE_TEXT);
	    db.exec("PRAGMA show_datatypes = on", null);
	    System.out.println("==== local callback ====");
	    db.exec("select * from sqlite_master", new SqlliteJniTest());
	    ...
	    db.exec("select myregfunc(TEST.firstname) from TEST", new SqlliteJniTest());
	    System.out.println("==== call aggregate function ====");
	    db.exec("select myaggfunc(TEST.firstname) from TEST", new SqlliteJniTest());
	    System.out.println("==== compile/step interface ====");
	    SqlliteJniTest cb = new SqlliteJniTest();
